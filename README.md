...
The sentence-level timestamps this relies on are additive metadata only (`data/chunks_contextual_with_timestamps.json`, a per-chunk `sentences` array layered on top of the committed `chunks_contextual.json`). `python -m seerah.ingest.sentence_timestamps` can add or remove this layer on an already-built index without any re-embedding, and it never touches embeddings or BM25 scoring, so retrieval quality is unaffected either way.
...
**Deployment (live)**: the frontend is a pure static build (`npm run build`), deployed to **Cloudflare Workers** (static assets, not a container — a static SPA has no server-side logic to containerize, so Docker only ever matters for the local `docker-compose` story, never for this half of production) at [seerah-ai.ibrahimumair900.workers.dev](https://seerah-ai.ibrahimumair900.workers.dev). It rebuilds and redeploys automatically on every push to `main` (`wrangler deploy --assets`, triggered by Cloudflare's own GitHub integration — no separate CI file in this repo). `VITE_API_URL` is set as a build-time variable in the Cloudflare dashboard, pointing at the backend below.

The backend + Qdrant + Postgres + Grafana run together via the same `docker-compose.yml`, on an **AWS Lightsail** VM (2 vCPU / 4GB, Ubuntu) — the exact stack described in "Containerization" below, unmodified for production. `caddy` (the `--profile proxy` service) is the only piece that's actually different from local dev: it's what's running there, terminating real TLS for `api.seerahaiapp.tech` via Let's Encrypt. A backend code change is deployed manually (`git pull && docker compose build app && docker compose up -d app` on the VM) rather than auto-deployed — deliberate, since every `/ask` call costs real OpenAI money and a bad deploy failing silently in the background is worse than a deploy step you have to remember to run. See "TLS / going public" under Containerization below for how the Let's Encrypt piece works.

## Monitoring & analytics

Every `/ask` call writes one row to Postgres' `conversations` table (question, answer, sources, search log, model, prompt/completion/total tokens, cost, overall response time, and `citation_time` — the citation/timestamp-refinement call's own duration, timed separately starting only *after* the main answer is already generated, so it isolates timestamp-lookup latency instead of folding it into the total — plus the `response_id`/`previous_response_id` pair that reconstructs multi-turn threads); every `/feedback` call writes to `feedback`, and every `/feedback/{id}/timestamp` call writes to its own `timestamp_feedback` table — kept separate rather than a column on `feedback` since the two rate different things (answer quality vs. citation timestamp precision) and a user may only rate one of them for a given turn. All three are linked by `conversation_id`; schema and helpers live in `seerah/db.py`.
...
11 panels: recent conversations (table), model usage (bar), answer feedback and timestamp feedback (pie, one per rating type — see above), response time / avg token usage / cost / timestamp-generation time (time series), plus avg response time, avg timestamp-generation time, and total cost as single-number stat panels.

Access at `http://localhost:3000` (`admin`/`admin` locally — production sets a real password via `GRAFANA_ADMIN_PASSWORD` in `.env`, never the default). **Deliberately never exposed publicly**, even in production — it shows internal metrics (cost, question volume, raw feedback) that have no reason to be public, so reaching it on the live VM means SSH-tunneling in (`ssh -L 13000:localhost:3000 <host>`) rather than a direct URL.
